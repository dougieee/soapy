...
class atmos(object):
	...
	def __init__(self, soapyConfig):
		...
		self.singleScrns = self.config.singleScrns
		self.infScrns = self.config.infScrns
		self.doRandomScrns = self.config.randomScrns
		self.L0 = self.config.L0
		...
		if self.singleScrns==False and self.infScrns==False and self.doRandomScrns==False:
			raise Exception('WHAT KIND OF SCREENS YOU WANTING?!')

		# If required, generate some new Kolmogorov phase screens
		if self.singleScrns==True:
